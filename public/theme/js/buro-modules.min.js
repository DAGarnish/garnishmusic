...
jQuery(window).on('load', function() {
    jQuery('.mkd-accordion').each(function() {
        var $holder = jQuery(this);
        // Try to destroy jquery ui accordion if it exists
        try {
            if ($holder.hasClass('ui-accordion')) {
                $holder.accordion('destroy');
            }
        } catch(e) {}
        
        // Re-initialize as standard accordion
        var $titles = $holder.find('.mkd-title-holder');
        var $contents = $titles.next('.mkd-accordion-content');
        
        $holder.addClass('accordion ui-accordion ui-accordion-icons ui-widget ui-helper-reset');
        $titles.addClass('ui-accordion-header ui-state-default ui-corner-top ui-corner-bottom');
        $contents.addClass('ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom').hide();
        
        // Unbind previous handlers just in case
        $titles.off('mouseenter mouseleave click');
        
        $titles.each(function() {
            var $title = jQuery(this);
            $title.on('mouseenter mouseleave', function() {
                $title.toggleClass('ui-state-hover');
            });
            $title.on('click', function() {
                if (!$title.hasClass('ui-state-active')) {
                    // Close others
                    $titles.not($title).removeClass('ui-accordion-header-active ui-state-active ui-corner-bottom').addClass('ui-state-default');
                    $contents.not($title.next()).slideUp(400, function(){ jQuery(this).removeClass('ui-accordion-content-active'); });
                    // Open this
                    $title.addClass('ui-accordion-header-active ui-state-active ui-corner-bottom').removeClass('ui-state-default');
                    $title.next().slideDown(400, function(){ jQuery(this).addClass('ui-accordion-content-active'); });
                } else {
                    // Close this
                    $title.removeClass('ui-accordion-header-active ui-state-active ui-corner-bottom').addClass('ui-state-default');
                    $title.next().slideUp(400, function(){ jQuery(this).removeClass('ui-accordion-content-active'); });
                }
            });
        });
    });
